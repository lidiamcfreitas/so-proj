...
int main(int argc, char* args[]){
    ...
    if (argc <= 1) {
        printf("por favor insira o nome do ficheiro ao executar o programa\n");
        return -1;
    }
    ...
    for(i=0;i<K;i++){
        linetogo[i] = i*(PORTION) * (sizeof(char)*10);
    ...
    if (fd == -1) {
        ...
        for (i=0; i< END; i++) {
            ...
            if (nbytes == 0){
                ...
                printf("Number of lines read by the last thread: %d\n", count);
                pthread_exit ((void*)-1);
            }
